...
import { getComponentPinoutCorrected, getPinPositionCorrected } from './pinout-data-corrected';
...
function parseConnectionPoint(
  ...
  const pinout = getComponentPinoutCorrected(part.type);
  ...
  let pinPos = getPinPositionCorrected(part.type, pin);
  
  // Try without suffix if not found
  if (!pinPos && pin.includes('.')) {
    const basePin = pin.split('.')[0];
    pinPos = getPinPositionCorrected(part.type, basePin);
    if (pinPos) {
